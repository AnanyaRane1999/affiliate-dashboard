...
const MOCK_ALERTS: Alert[] = [
  { id: 1, type: "spend_spike",      message: "Camping Anfänger campaign CPC exceeded €2.50 threshold",         severity: "critical", resolved: false, created_at: "2026-03-27T09:30:00Z" },
  { id: 2, type: "tracking_failure", message: "GA4 tracking not firing on kitesurfen.starterkit.de/starter-kit", severity: "warning",  resolved: false, created_at: "2026-03-27T08:15:00Z" },
  { id: 3, type: "link_broken",      message: "2 affiliate links returning 404 on Camping niche",                severity: "warning",  resolved: false, created_at: "2026-03-27T07:00:00Z" },
  { id: 4, type: "budget_low",       message: "Kitesurfen niche budget is 80% consumed this month",             severity: "info",     resolved: false, created_at: "2026-03-26T18:00:00Z" },
  { id: 5, type: "agent_error",      message: "Research Agent failed to connect to SerpAPI — retrying",         severity: "warning",  resolved: true,  created_at: "2026-03-26T12:00:00Z" },
];

const SEVERITY_CONFIG: Record<AlertSeverity, { label: string; iconClass: string; badgeClass: string; bg: string; icon: React.ElementType }> = {
  critical: { label: "Critical", iconClass: "text-red-400",    badgeClass: "text-red-400 border-red-700",    bg: "bg-red-900/20 border-red-700/50",    icon: AlertOctagon },
  warning:  { label: "Warning",  iconClass: "text-yellow-400", badgeClass: "text-yellow-400 border-yellow-700", bg: "bg-yellow-900/20 border-yellow-700/50", icon: AlertTriangle },
  info:     { label: "Info",     iconClass: "text-blue-400",   badgeClass: "text-blue-400 border-blue-700",  bg: "bg-blue-900/20 border-blue-700/50",   icon: Info },
};
...
export default function AlertsPage() {
  ...
  useEffect(() => {
    const critical = displayAlerts.filter((a) => a.severity === "critical" && !a.resolved).length;
    setUnreadAlertCount(critical);
  }, [displayAlerts, setUnreadAlertCount]);

  const resolveMutation = useMutation({
    mutationFn: (id: number) => api.patch(API_ROUTES.alertResolve(id)),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["alerts"] }),
    onError: () => queryClient.invalidateQueries({ queryKey: ["alerts"] }),
  });
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 mt-1">{unresolvedCount} unresolved — {criticalCount} critical</p>
        </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        {(["all", "critical", "warning", "info"] as const).map((f) => (
          <button
            ...
            className={clsx(
              ...
              severityFilter === f ? "bg-blue-600 text-white" : "bg-gray-800 text-gray-400 hover:bg-gray-700"
            )}
          >
        ...
        <button
          ...
          className={clsx(
            ...
            showResolved ? "bg-gray-600 text-white" : "bg-gray-800 text-gray-400 hover:bg-gray-700"
          )}
        >
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
        <div className="flex items-center gap-2 px-6 py-4 border-b border-gray-800">
          ...
          <span className="ml-auto text-gray-500 text-sm">{filtered.length} alerts</span>
        </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className={clsx("text-xs font-medium px-2 py-0.5 rounded-full border", config.badgeClass)}>
                          {config.label}
                        </span>
                        <span className="text-gray-500 text-xs">{alert.type.replace(/_/g, " ")}</span>
                        {alert.resolved && (
                          <span className="text-xs bg-gray-700 text-gray-400 px-2 py-0.5 rounded-full">Resolved</span>
                        )}
                      </div>
