...
import { TrendingUp, TrendingDown, Globe, Activity, Megaphone, AlertTriangle } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const KPI_CARDS = [
    { label: "Total Niches", value: displayNiches.length.toString(), change: `${liveNiches.length} approved`, up: true, icon: Globe, color: "text-blue-400" },
    { label: "Ad Spend", value: `€${totalSpent.toFixed(0)}`, change: `of €${totalBudget.toFixed(0)} allocated`, up: true, icon: Megaphone, color: "text-purple-400" },
    { label: "Active Alerts", value: unresolvedAlerts.toString(), change: unresolvedAlerts > 0 ? "needs attention" : "all clear", up: unresolvedAlerts === 0, icon: AlertTriangle, color: unresolvedAlerts > 0 ? "text-red-400" : "text-green-400" },
    { label: "Active Niches", value: liveNiches.length.toString(), change: `${displayNiches.length} total discovered`, up: true, icon: TrendingUp, color: "text-orange-400" },
  ];
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl">
          ...
          <div className="divide-y divide-gray-800">
            {displayNiches.length === 0 ? (
              <div className="px-6 py-8 text-center text-gray-500 text-sm">Loading niches...</div>
            ) : (
              displayNiches.sort((a, b) => (b.score || 0) - (a.score || 0)).slice(0, 5).map((niche) => (
                <div key={niche.id} className="flex items-center justify-between px-6 py-3 hover:bg-gray-800/30 cursor-pointer" onClick={() => router.push(`/dashboard/niches/${niche.id}`)}>
                  <div>
                    <p className="text-white text-sm font-medium">{niche.name}</p>
                    <p className="text-gray-500 text-xs mt-0.5">Score: {niche.score?.toFixed(2)} | Budget: €{niche.recommended_budget}</p>
                  </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl">
          ...
          <div className="divide-y divide-gray-800">
            {displayLogs.length === 0 ? (
              <div className="px-6 py-8 text-center text-gray-500 text-sm">Loading agent logs...</div>
            ) : (
              displayLogs.slice(0, 5).map((log: any) => (
                <div key={log.id} className="flex items-start gap-3 px-6 py-3">
                  <span className="text-base mt-0.5">✅</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-white text-sm font-medium truncate">{log.agent_name} — {log.action}</p>
                    <p className="text-gray-500 text-xs mt-0.5 truncate">{log.result_summary || log.input_summary || "—"}</p>
                  </div>
                </div>
              ))
            )}
      ...
      <div className="flex items-center gap-3 bg-green-900/20 border border-green-700/50 rounded-xl px-5 py-4">
        ...
        <p className="text-green-400 text-sm"><strong>Live data active</strong> — Connected to backend API. Data refreshes every 30 seconds.</p>
      </div>
    </div>
  );
}
